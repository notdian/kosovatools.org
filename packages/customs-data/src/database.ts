...
export class CustomsDataService {
  ...
  static async initializeData({
    ...
    try {
      ...
      const normalized: CustomsRecord[] = data.map((entry) => ({
        ...
        excise: Number.isFinite(Number(entry.excise)) ? Number(entry.excise) : 0,
  ...
  private static async getSubtreesForHits(
    ...
    const highlightMap = new Map(hits.map((hit) => [hit.id, hit.highlight || null]))
    const allRecords = new Map<string, CustomsRecord>()
  ...
  private static async searchByDescriptionHits(query: string): Promise<MiniSearchHit[]> {
